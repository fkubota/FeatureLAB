...
class histogram_mod(QG.QWidget):
    def __init__(self, parent=None):
        self.tabV = []
        self.tab_id = -1

        super(histogram_mod, self).__init__(parent)  # superclassのコンストラクタを使用。
        self.setWindowTitle("histogram")

        # graph
        self.w_hist_plot = pg.GraphicsWindow()
        self.w_hist_plot.resize(300, 200)
        self.w_hist_plot.setBackground('#FFFFFF00')
        self.p0_hist = self.w_hist_plot.addPlot()
        self.p0_hist.showGrid(x=True, y=True, alpha=0.7)


        # wdiget
        self.btn0 = QG.QPushButton('add Plot')
        self.btn0.clicked.connect(self.add_Data)
        self.btn0.clicked.connect(self.update_hist_cb)
        ...
        self.hsplitter0.addWidget(self.w_hist_plot)
    ...
    def add_Data(self):
        ...
        # event
        self.tab_new.cb0.currentIndexChanged.connect(self.hist_setting_update)
        self.tab_new.cb2.currentIndexChanged.connect(self.hist_setting_update)
        self.tab_new.btn.clicked.connect(self.hist_setting_update)
        self.tab_new.le0.editingFinished.connect(self.hist_setting_update)
        self.tab_new.le1.editingFinished.connect(self.hist_setting_update)
        self.tab_new.btn_color.clicked.connect(self.hist_change_color)
        self.tab_new.btn_color.clicked.connect(self.hist_setting_update)
        self.tab_new.check.stateChanged.connect(self.hist_setting_update)


        # plotitem
        # self.tab_new.plot_hist = pg.FillBetweenItem()
        # self.p0_hist.addItem(self.tab_new.plot_hist)
        self.tab_new.plot_hist = self.p0_hist.plot()

        # update comnobox
        self.update_hist_cb()


    def hist_setting_update(self):
        pass
    def hist_change_color(self):
        pass
    def update_hist_cb(self):
        pass
